...
"""RETIRED SCRIPT — DO NOT EXECUTE.

This script was an early IF-1 remediation seed tool targeting the legacy
security_lifecycle schema (stock_id, otc_first_date, mainboard_date,
mainboard_type, ...) which predates the point-in-time model.

The production schema is now:
    stock_id, listed_from, listed_to, market, source_type, ...

The canonical seed tool for the current schema is:
    scripts/seed_security_lifecycle.py

Running this script against the current database will fail due to schema
mismatch and may corrupt the IF-1 remediation audit trail if modified.

Retired: 2026-06-07 (P1-DATA-FOLLOWUP)
Authority: SPEC-P1-DATA-REMEDIATION-v1
"""

raise SystemExit(
    "scripts/ingest_security_lifecycle.py is RETIRED. "
    "Use scripts/seed_security_lifecycle.py for the current schema."
)

# -- Original source preserved below for audit trail -------------------------
# fmt: off
# ruff: noqa
# #!/usr/bin/env python3
# # scripts/ingest_security_lifecycle.py
